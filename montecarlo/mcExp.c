...
float angleToWall(float phi, int wall)
{
	...
	float numerator = dy*sin(phi) + dx*cos(phi);
...
{
  int wall = getClosestWallForward(p_x,p_y,phi);
  if(wall == -1)
  {
    return 0; //Particle facing away from map assign it 0 weight.
  }

	float expectedDepth = getClosestWallForwardDistance(p_x,p_y,phi,wall);
...
void drawMap()
{
  ...
  nxtDisplayCenteredTextLine(2, "distance: %d", SensorValue(sonar_sensor) - 6);
	// Display the map
...
{
	// Draw new position
	nxtSetPixel((int) (p_x/DISPLAY_SCALE) + OFFSET, (int) (p_y/DISPLAY_SCALE) + OFFSET);
}
